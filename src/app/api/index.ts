...
import type { CommonRequestConfig, CommonRequestInterceptorsConfig, APIResponseType, outerAPIResponseType, IResponse } from './APICLIENT';
...
class ApiClient {
		...
		this.axiosInstance.interceptors.request.use(
			...
			//): InternalAxiosRequestConfig<any>|Promise<InternalAxiosRequestConfig<any>> => {
				...
				if (requestConfig.url !== '/api/login') {
					requestConfig.headers['Authorization'] = `Bearer ${window.spa.utils.getLocalStorage('access_token')}`;
				}
				...
				if ( url && !allowDuplicate ) {
					...
					const hashInt = window.spa.utils.hashStringTo32BitInteger(url.concat(JSON.stringify(hashData)));
					// 리스트에 중복코드가 있는지 체크
			...
			(error: any): any => {
				if (error.config) {
					const {
						url,
						unresolvedHash,
					} = error.config as CommonRequestConfig;
	...
	// 공통 response타입이 정해진 IResponse를 사용한 함수
